...
def apply_manual_trades(current_state, planned_trades, prices):
    ...
    if not prices:
         ...
         # Return current state if prices are missing? Or raise error? Returning current for now.
    ...
    for trade in sells:
        ...
        # Apply estimated slippage/commission (match portfolio_manager logic if possible)
        ...
        # Only process if proceeds are meaningful (handle potential tiny negative due to costs)
        if proceeds > -1e-6: # Allow slightly negative for rounding
            new_cash += proceeds
            new_positions[ticker] -= shares_to_sell
            if new_positions[ticker] <= 0: # Remove if shares go to zero or below (shouldn't happen)
    ...
    # Optional: Sort buys (e.g., by ticker)
    buys.sort(key=lambda x: x['ticker'])
    ...
    for trade in buys:
        ...
        if new_cash >= cost:
            ...
            # Log a clear warning about potential divergence.
            ...
            # Simulate it anyway for the file state, allowing cash to go negative if needed.
            new_cash -= cost
            new_positions[ticker] = new_positions.get(ticker, 0) + shares_to_buy
...
if __name__ == "__main__":
    ...
    if not os.path.exists(orders_file):
        ...
        # We should still save the loaded state back to ensure format consistency
    ...
    try:
        ...
        if not isinstance(planned_trades, list) or not all(isinstance(t, dict) and 'ticker' in t and 'shares' in t for t in planned_trades):
    ...
    if not planned_trades:
        ...
        # Clean up empty orders file? Optional.
    ...
    latest_prices = {}
    # Use datetime and timedelta which are now imported
    ...
    for i in range(days_to_check):
         ...
         check_date_dt_only = check_date.normalize() # Get date part only for index checking

         logging.debug(f"Trying to fetch prices for date: {check_date_dt_only.strftime('%Y-%m-%d')}")
         ...
         for ticker in tickers_in_trades:
             ...
             if price_data is not None and not price_data.empty:
                  ...
                  if check_date_dt_only in price_data.index:
                       price = price_data.loc[check_date_dt_only]['Close']
                       if pd.notna(price) and price > 0:
                           temp_prices[ticker] = price
                       else: # Price is NaN or zero
                           all_prices_found = False
                           missing_ticker_on_date = f"{ticker} (Invalid Price: {price})"
                           logging.debug(f"Invalid price found for {ticker} on {check_date_dt_only}")
                           break # Stop checking this date
                  else: # Date missing in index
                      all_prices_found = False
                      missing_ticker_on_date = f"{ticker} (Date Missing)"
                      logging.debug(f"Date {check_date_dt_only} missing for {ticker}")
                      break # Stop checking this date
             else: # File missing or empty
                 all_prices_found = False
                 missing_ticker_on_date = f"{ticker} (File Missing/Empty)"
                 logging.debug(f"Data file missing or empty for {ticker}")
                 break # Stop checking this date

         if all_prices_found:
              latest_prices = temp_prices
              found_prices_date = check_date_dt_only
             ...
             logging.warning(f"Price check failed for date {check_date_dt_only.strftime('%Y-%m-%d')}. {reason}")
    ...
    save_state = {
        ...
        'positions': {str(k): int(v) for k, v in new_state.get('positions', {}).items()}
    }
